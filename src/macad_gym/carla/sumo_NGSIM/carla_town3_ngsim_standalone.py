#!/usr/bin/env python3
"""
CARLA Town3 Edge3 NGSIM车辆轨迹重现系统 - 精确横向控制增强版
基于 enhanced_lateral_control.py 的完整功能集成
...
class CarlaTown3NgsimController:
    """CARLA Town3 NGSIM轨迹重现控制器 - 完全独立版本"""
    ...
    def __init__(self):
        ...
        # CARLA Town3专用参数 - Edge3路线
        self.carla_params = {
            'vehicle_half_width': 0.9,      # 车辆半宽
            'safety_margin': 1.0,           # 安全边距
            'enable_debug': True,           # 调试开关
            
            # CARLA Town3道路边界 (Edge3)
            'road_boundaries': {
                'min_x': -83.0,             # 道路左边界 (最左侧车道)
                'max_x': -73.0              # 道路右边界 (最右侧车道)
            },
            
            # CARLA Town3路线参数 (Edge3)
            'route': {
                'edge3_y_start': 10.41,     # edge3起点Y (车辆起始位置)
                'edge3_y_end': 124.92,      # edge3终点Y
                'total_length': 114.52      # 总路线长度
            },
            
            # CARLA车道中心坐标 (基于Edge3的车道位置)
            'lane_centers': {
                1: -81.4,    # NGSIM Lane 1 → 左侧车道 (lane 3_2) - 快车道
                2: -77.9,    # NGSIM Lane 2 → 中间车道 (lane 3_1)
                3: -74.4     # NGSIM Lane 3 → 右侧车道 (lane 3_0) - 慢车道
            },
            
            # 各车道安全偏移范围
            'lane_offsets': {
                1: {'min': -1.5, 'max': +1.5},  # 左侧车道 (快车道)
                2: {'min': -1.5, 'max': +1.5},  # 中间车道
                3: {'min': -1.5, 'max': +1.5}   # 右侧车道 (慢车道)
            }
        }
    ...
    def analyze_lane_x_ranges(self, df):
        ...
        for lane_id in sorted(df['Lane_ID'].unique()):
            ...
            # 确保Local_X是数值类型 - 先清理逗号
            local_x_raw = lane_data['Local_X']
            if isinstance(local_x_raw.iloc[0], str):
                local_x_raw = local_x_raw.str.replace(',', '')
            local_x_series = pd.to_numeric(local_x_raw, errors='coerce')
            # 移除NaN值
            local_x_series = local_x_series.dropna()
            # 转换为米
    ...
    def setup_coordinate_transform(self, df):
        ...
        # 确保坐标列为数值类型并转换为米 - 先清理逗号
        local_y_raw = df['Local_Y']
        if isinstance(local_y_raw.iloc[0], str):
            local_y_raw = local_y_raw.str.replace(',', '')
        local_y_values = pd.to_numeric(local_y_raw, errors='coerce').dropna() * self.feet_to_meters
        
        local_x_raw = df['Local_X'] 
        if isinstance(local_x_raw.iloc[0], str):
            local_x_raw = local_x_raw.str.replace(',', '')
        local_x_values = pd.to_numeric(local_x_raw, errors='coerce').dropna() * self.feet_to_meters
        ...
        carla_y_start = self.carla_params['route']['edge3_y_start']  # 10.41m
        carla_y_end = carla_y_start + actual_travel_distance  # 动态计算终点
        ...
        self.transform_params = {
            ...
            'carla_y_range': (carla_y_start, carla_y_end),  # 基于实际行驶距离
        ...
        print(f"🗺️  CARLA道路X: {self.transform_params['carla_x_range'][0]:.1f}m ~ {self.transform_params['carla_x_range'][1]:.1f}m")
    ...
    def map_coordinates_enhanced_lateral(self, local_y, local_x, lane_id):
        ...
        road_min_x = self.carla_params['road_boundaries']['min_x']       # -83.0m
        road_max_x = self.carla_params['road_boundaries']['max_x']       # -73.0m
        
        # 为每个车道计算考虑车身宽度的安全偏移限制 (CARLA Edge3适配)
        # 原理：确保车辆边缘(center±0.9m)不超出道路边界(±0.1m安全边距)
        lane_safe_offsets = {
            # Lane 1 (-81.4m): 左侧快车道，左边界限制更严格
            1: {
                'min': -1.0,  # 右侧可以偏移更多
                'max': (road_max_x - safety_margin - vehicle_half_width) - (-81.4)  # -73.0-0.1-0.9-(-81.4) = +6.4m → 限制到 +0.6m
            },
            # Lane 2 (-77.9m): 中间车道，相对对称偏移
            2: {
                'min': -0.8, 
                'max': +0.8
            },
            # Lane 3 (-74.4m): 右侧慢车道，右边界限制更严格
            3: {
                'min': (road_min_x + safety_margin + vehicle_half_width) - (-74.4),  # -83.0+0.1+0.9-(-74.4) = -7.6m → 限制到 -0.6m
                'max': +1.0  # 左侧可以偏移更多
            }
        }
        ...
        if lane_id in lane_safe_offsets:
            ...
            if (self.lateral_params['enable_debug'] and 
                ...
                print(f"🔍 Lane{lane_id}车身边界验证: 右边缘{vehicle_right_edge_min:.2f}m ~ 左边缘{vehicle_left_edge_max:.2f}m "
        ...
        # 第六步：基于车身边缘的最终边界检查 ⭐
        # 计算车辆边缘位置
        vehicle_left_edge = carla_x - vehicle_half_width   # 车辆左边缘
        vehicle_right_edge = carla_x + vehicle_half_width  # 车辆右边缘
        
        # 道路边界（包含安全边距）
        road_boundary_min = road_min_x + safety_margin  # -82.90m
        road_boundary_max = road_max_x - safety_margin  # -73.10m
    ...
    def create_carla_route_file(self, route_file):
        ...
        vtype.set("maxSpeed", "25.0")  # 适当提高最大速度
        vtype.set("accel", "2.6")
        vtype.set("decel", "4.5")
        vtype.set("emergencyDecel", "9.0")  # 紧急制动
        vtype.set("sigma", "0.5")
        vtype.set("tau", "1.0")  # 反应时间
        vtype.set("speedFactor", "1.0")
        vtype.set("speedDev", "0.1")
        vtype.set("guiShape", "passenger")
        vtype.set("color", "1,0.8,0")  # 金黄色
        vtype.set("carFollowModel", "Krauss")  # 跟车模型
        vtype.set("laneChangeModel", "LC2013")  # 变道模型
    ...
    def add_vehicle(self, vehicle_id, frame_data, lane_id):
        ...
        try:
            ...
            if not positioned:
                try:
                    # CARLA Edge3 车道映射：1=lane3_2(左), 2=lane3_1(中), 3=lane3_0(右)
                    target_lane_index = {1: 2, 2: 1, 3: 0}.get(lane_id, 1)
                    traci.vehicle.moveToXY(
                        vehID=sumo_vehicle_id,
                        edgeID="3",
            ...
            # 车辆信息显示 (增强版)
            carla_lane = {1: "左侧车道(3_2-快车道)", 2: "中间车道(3_1)", 3: "右侧车道(3_0-慢车道)"}.get(lane_id, "unknown")
            ...
            print(f"   📏 最终车身: 左边缘{final_left_edge:.2f}m ~ 右边缘{final_right_edge:.2f}m (偏移:{lateral_deviation:.2f}m)")
        ...
        try:
            ...
            positioning_strategies = [
                ("moveToXY_full", lambda: traci.vehicle.moveToXY(
                    vehID=sumo_vehicle_id, edgeID="3", laneIndex=-1,
                ...
                ("moveToXY_lane_specific", lambda: traci.vehicle.moveToXY(
                    ...
                    laneIndex={1: 2, 2: 1, 3: 0}.get(frame_data['Lane_ID'], 1),
                    x=target_x, y=target_y))
            ]
    ...
    def run_simulation(self, csv_file, net_file):
        """运行主仿真"""
        print(f"\n" + "="*80)
        print(f"🎯 CARLA Town3 Edge3 NGSIM轨迹重现系统 - 精确横向控制增强版")
        ...
        print(f"🛡️ 道路边界: {self.carla_params['road_boundaries']['min_x']:.1f}m ~ {self.carla_params['road_boundaries']['max_x']:.1f}m")
        ...
        print(f"🚗 车道映射: NGSIM Lane1,2,3 → CARLA 左侧/中间/右侧车道 (快车道/中车道/慢车道)")
        ...
        try:
                    ...
                    for vehicle_id in new_vehicles:
                        if vehicle_id in self.vehicle_trajectories:
                            ...
                            if len(frame_data) > 0:
                                ...
                                if self.add_vehicle(vehicle_id, row, row['Lane_ID']):
                                    added_count += 1
            ...
            print(f"  - 车辆边缘范围: 完全在道路内 ({self.carla_params['road_boundaries']['min_x']:.1f}~{self.carla_params['road_boundaries']['max_x']:.1f}m)")
            print(f"  - 边缘保护: ✅ 车身宽度考虑")
            ...
            print(f"\n🔍 继续观察 (30秒)...")
            for i in range(300):
                remaining = [v for v in self.active_vehicles.keys() 
                           if self.active_vehicles[v]['sumo_id'] in traci.vehicle.getIDList()]
                
                if not remaining:
                    print(f"ℹ️ 所有车辆已完成路线")
                    break
                
                if i % 50 == 0:
                    print(f"观察 +{i//10}s: 剩余 {len(remaining)}辆")
...
def main():
    """主函数"""
    print("🎯 CARLA Town3 NGSIM轨迹重现系统 - 精确横向控制增强版")
    print("✅ 车身边缘安全 + SUMO官方标准角度计算")
    print("🚗 车身宽度1.8m + 车辆边缘完全在道路内")
    print("🧭 SUMO官方角度: 基于位移向量 + 数值稳定性检查")
    print("🛡️ 按车道精确映射: Lane 1→左快车道, Lane 2→中车道, Lane 3→右慢车道")
    print("🔒 车辆边缘保护: 道路范围-83.0m ~ -73.0m")
    ...
    # 文件路径
    csv_file = "final_70.csv"
    net_file = "carla_town3.net.xml"
