...
class CarlaTown3NgsimController:
    ...
    def __init__(self):
        ...
        self.carla_params = {
            ...
            # CARLA Town3道路边界 (Edge3) - training_Town03.net.xml坐标
            'road_boundaries': {
                'min_x': 204.0,  # 道路右边界 (最右侧车道3_5)
                'max_x': 214.0  # 道路左边界 (最左侧车道3_3)
            },
            ...
            # CARLA车道中心坐标 (基于Edge3的车道位置) - training_Town03.net.xml
            'lane_centers': {
                1: 205.5,  # NGSIM Lane 1 → 右侧车道 (lane 3_5) - 慢车道
                2: 209.0,  # NGSIM Lane 2 → 中间车道 (lane 3_4)
                3: 212.5  # NGSIM Lane 3 → 左侧车道 (lane 3_3) - 快车道
            },
    ...
    def map_coordinates_enhanced_lateral(self, local_y, local_x, lane_id):
        ...
        lane_safe_offsets = {
            ...
            1: {
                'min': -1.0,  # 右侧可以偏移更多
                'max': (road_max_x - safety_margin - vehicle_half_width) - (-81.4)
                # -73.0-0.1-0.9-(-81.4) = +6.4m → 限制到 +0.6m
            },
            ...
            3: {
                'min': (road_min_x + safety_margin + vehicle_half_width) - (-74.4),
                # -83.0+0.1+0.9-(-74.4) = -7.6m → 限制到 -0.6m
                'max': +1.0  # 左侧可以偏移更多
            }
        }
    ...
    def calculate_vehicle_angle(self, vehicle_id, current_pos, prev_pos=None):
        ...
        if angle_diff > 45:  # 45度阈值
            ...
            if self.lateral_params.get('enable_debug', False):
                print(f"🔧 车辆{vehicle_id}角度平滑: {last_angle:.1f}° → {sumo_angle:.1f}° (原始:{math.degrees(math.atan2(dy, dx)) + 90:.1f}°)")
    ...
    def add_vehicle(self, vehicle_id, frame_data, lane_id):
        ...
        try:
            ...
            if not positioned:
                try:
                    # CARLA Edge3 车道映射 (training_Town03.net.xml)：1=lane3_5(右), 2=lane3_4(中), 3=lane3_3(左)
...
def main():
    ...
    print("🔒 车辆边缘保护: 道路范围204.0m ~ 214.0m (training_Town03.net.xml)")
