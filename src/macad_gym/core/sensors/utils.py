...
def preprocess_image(image, config):
    ...
    # use_semantic_segmentation_camera = config["use_semantic_segmentation_camera"]
    ...
    if use_depth_camera:
        ...
        if config["camera_type"] == "semseg":
            image.convert(carla.ColorConverter.CityScapesPalette)
        data = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
        # size_y:image.height , size_x:image_width
        # print("image data shape:", data.shape)
        data = np.reshape(data, (image.height, image.width, 4))
        data = data[:, :, :3]
        data = data[:, :, ::-1]
        data = cv2.resize(data, (x_res, y_res), interpolation=cv2.INTER_AREA)
        # print("image data shape after cv2.resize:",data.shape)
        # data = (data.astype(np.float32) - 128) / 128
        # image data shape: (153600,)   ,
        # image data shape after cv2.resize: (120, 320, 3)
