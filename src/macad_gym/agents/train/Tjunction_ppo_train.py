...
def env_creator(env_config):
    ...
    # 添加 SUMO 相關設置
    modified_config["env"]["enable_sumo_cosim"] = True  # 啟用 SUMO 聯合仿真
    modified_config["env"]["sumo_gui"] = True  # 啟用 SUMO GUI
    modified_config["env"]["verbose"] = True  # 啟用詳細日誌
    
    # 🎯 關鍵：配置NGSIM背景交通流
    modified_config["env"]["enable_ngsim_background"] = True  # 启用NGSIM背景交通
    modified_config["env"]["ngsim_csv_file"] = "final_60.csv"  # NGSIM数据文件名
    modified_config["env"]["sumo_to_carla_sync"] =  True # SUMO和CARLA同步
    ...
    print("CARLA-SUMO-NGSIM Co-simulation System Enabled")
...
analysis = tune.run(
    "PPO",
    name="Tjunction_ppo_LSTM_advr5_DEC8",
    stop={
