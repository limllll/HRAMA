...
class CarlaImitationModelContinuous(Model):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, **kwargs):
        super(CarlaImitationModelContinuous, self).__init__(obs_space, action_space, num_outputs, model_config, name)
    ...
    def _build_layers_v2(self, inputs, num_outputs, options):
        ...
        with tf.name_scope("carla_out"):
            ...
            # 第一个值 (output[0]) -> 范围 [-0.75, 0.75]
            throttle = 0.5 * output[:, 0]  # 将 [-1, 1] 映射到 [-0.75, 0.75]

            # 第二个值 (output[1]) -> 范围 [-0.5, 0.5]
            steer = 0.5 * output[:, 1]  # 将 [-1, 1] 映射到 [-0.5, 0.5]

            # 合并两个值
            output = tf.stack([throttle, steer], axis=1)
        return output, last_layer

def register_carla_imitation_model_continous():
    print(ModelCatalog)
    print("type:", type(ModelCatalog))
    print(dir(ModelCatalog))
    ModelCatalog.register_custom_model("carla_imitation_model_continuous", CarlaImitationModelContinuous)

class CarlaImitationModel(Model):
    """
    结构消融由 custom_options["structural_ablation"] 控制：
      - image_only: 仅图像 512 → MLP(512,256,256)
      - image_state: 图像 + 状态编码(64)，融合维 576
      - image_state_lidar: 图像 + 状态 + 点云(256)，融合维 832；无 RLlib LSTM（由训练配置 use_lstm=False）
      - full: 与 image_state_lidar 相同骨干 + 训练端 use_lstm=True
    """
    def _build_layers_v2(self, inputs, num_outputs, options):
        hiddens_vision = []
        # 🎯 车辆状态编码配置
        hiddens_measure = [128, 128, 64]
        # 🎯 融合后 MLP (最后 256 维；是否再接 LSTM 由 model.use_lstm 决定)
        hiddens_concat_out = [512, 256, 256]
        activation = tf.nn.relu

        custom_opts = options.get("custom_options", {})
        structural = custom_opts.get("structural_ablation", "full")
        _valid = ("full", "image_only", "image_state", "image_state_lidar")
        if structural not in _valid:
            structural = "full"

        use_state = structural in ("image_state", "image_state_lidar", "full")
        use_lidar = structural in ("image_state_lidar", "full")
        ...
        print("=" * 80)
        print("[结构消融 structural_ablation={}] use_state={} use_lidar={}".format(
            structural, use_state, use_lidar))
        print("=" * 80)

        metrics_encoded = None
        if use_state:
            print("[车辆状态编码器] 输入: {}".format(metrics_in.get_shape()))
            with tf.name_scope("carla_metrics"):
                metrics_encoded = metrics_in
                for i, size in enumerate(hiddens_measure, 1):
                    metrics_encoded = slim.fully_connected(
                        metrics_encoded,
                        size,
                        weights_initializer=tf.keras.initializers.glorot_normal(),
                        activation_fn=activation,
                        scope="metrics_fc{}".format(i))
                    print("  - 车辆状态FC{} → {}维: {}".format(i, size, metrics_encoded.get_shape()))

        fusion_parts = [vision_in]
        if use_state:
            fusion_parts.append(metrics_encoded)
        if use_lidar:
            fusion_parts.append(lidar_in)

        print("\n[多模态融合输入]")
        print("  - vision_in: {}".format(vision_in.get_shape()))
        if use_state:
            print("  - metrics_encoded: {}".format(metrics_encoded.get_shape()))
        if use_lidar:
            print("  - lidar_in: {}".format(lidar_in.get_shape()))

        with tf.name_scope("carla_fusion"):
            if len(fusion_parts) == 1:
                last_layer = fusion_parts[0]
            else:
                last_layer = tf.concat(fusion_parts, axis=1)
            print("  - 拼接后特征: {}".format(last_layer.get_shape()))

            for i, size in enumerate(hiddens_concat_out, 1):
                last_layer = slim.fully_connected(
                    last_layer,
                    size,
                    weights_initializer=tf.keras.initializers.glorot_normal(),
                    activation_fn=activation,
                    scope="fusion_fc{}".format(i))
                print("  - 融合FC{} → {}维: {}".format(i, size, last_layer.get_shape()))
        
        # 输出层；是否接 RLlib LSTM 由训练配置 model.use_lstm 决定
        print("\n[输出层]")
        print("  - 特征层输出: {}".format(last_layer.get_shape()))
        ...
        with tf.name_scope("output"):
            output = slim.fully_connected(
                ...
                activation_fn=None,
                scope="action_out")
            print("  - 动作输出: {}".format(output.get_shape()))
        
        print("="*80)
        print("[多模态DRL策略网络] 网络构建完成")
        print("="*80 + "\n")
        
        # 返回output和last_layer，RLlib会在last_layer之后添加LSTM
        return output, last_layer
...
class CarlaModelv0(Model):
    ...
    Define the layers of a custom model.
            ...
            """
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, **kwargs):
        super(CarlaModelv0, self).__init__(obs_space, action_space, num_outputs, model_config, name)
